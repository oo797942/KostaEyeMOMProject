...
// 				alert(pass);
				location.href="deleteDonation.do?b_no="+num+"&b_cate="+cate;
			}
			else
			{
		...
		$('#goupdate').click(function(){
			...
			location.href="updateDonation.do?b_no="+num;
		});
		
		$('#donationfin').click(function(){
			var a = confirm("나눔을 완료하시겠습니까?");
			var num = $('#b_no').val();
			if(a == true){
				location.href="donationfin.do?b_no="+num;
			}
		})
				...
				<div id='tableall2'>			 <!-- 게시판 전체 div -->
					...
					<table class='boardtable'>   <!-- 테이블 전체 div -->
						<tr id='trcolor2'>
							...
							<!-- 게시글 제목 -->
							<th colspan="9" class='boardtd1'><label id='title'>[${bvo.b_scate}] ${bvo.b_title}</label></th>
							<!-- 게시판 카테고리 -->
							<th class='boardtd2'>
							<input type="hidden" id="cate" name="cate" value= "${bvo.b_cate}">
								<label id='cate'>아나바다</label> 
							</th>
						</tr>
						<tr id='trcolor2'>
...
<!-- 							<img alt="이미지가 올라갈 영역임" src="#" border="2"> -->
					...
					<div id='comment2'>				
						...
						<div style="float: right;">
							...
							<c:if test="${bvo.b_scate eq '나눔'}" >
							<input type='button' value='나눔완료' id="donationfin" class='optionbtn'/>
							</c:if>
